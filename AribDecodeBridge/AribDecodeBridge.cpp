...
static const wchar_t T_AlnumMiddle[96]={
...
static wchar_t MapAlnumChar(int idx, bool middleSize) {
    if(idx < 0 || idx >= 96) return 0;
    return middleSize ? T_AlnumMiddle[idx] : T_Alnum[idx];
}
    ...
    for(int i=0;i<length;i++) {
        ...
        if(b>=0x21&&b<=0x7E) {
            ...
            if(IsDouble(cs)) {
                ...
                if(idx>=0&&idx<96) {
                    switch(cs) {
                    case CS::Alnum:   { wchar_t c=MapAlnumChar(idx,middleSize); if(c) PutChar(buf,pos,cap,c); break; }
        ...
        case 0x89: middleSize=true; break;
        case 0x8A: middleSize=false; break;
